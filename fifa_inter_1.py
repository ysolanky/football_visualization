...
## Start------- Goals
 ...
 \
        ...
        with col_goal_table:
            ...
            st.subheader("Goals table by minute")
...
fig_summary.add_trace(go.Scatter(x=final_goal['minute'], y=final_goal['summary'],mode='markers',text=final_goal['player'],
                    marker=dict(color=final_goal.possession_team_id,size=45,opacity=1),customdata=['possession_team']))
...
fig_summary.update_layout(font=dict(size=20),title="Goals scored by minute",xaxis_title="Time in minutes",)                 
st.write(fig_summary)
